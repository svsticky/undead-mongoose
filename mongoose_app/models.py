...
class Product(models.Model):
    ...
    vat = models.ForeignKey("VAT", on_delete=models.CASCADE, verbose_name="BTW")
...
class IDealTransaction(Transaction):
    ...
    status = models.IntegerField(
        choices=PaymentStatus.choices, default=PaymentStatus.OPEN
    )
    added = models.BooleanField(default=False)
    ...
    def __str__(self):
        return (
            "iDeal transaction { status = "
            + str(PaymentStatus(self.status).label)
            + ", added = "
            + str(self.added)
            + ", id = "
            + str(self.id)
...
class VAT(models.Model):
    percentage = models.IntegerField(
        validators=[
            MinValueValidator(
                limit_value=0, message="Percentage can't be lower than 0"
            ),
            MaxValueValidator(
                limit_value=100, message="Percentage can't be higher than 100"
            ),
        ]
    )
...
class ProductForm(forms.ModelForm):
    name = forms.CharField(
        widget=forms.TextInput(
            attrs={"placeholder": "Suikerwafel", "class": "form-control"}
        ),
        label=mark_safe('<label class="form-label">Name</label>'),
    )
    price = forms.DecimalField(
        widget=forms.TextInput(attrs={"placeholder": "15.15", "class": "form-control"}),
